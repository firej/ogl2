...
File::File(const char *Fname, char Omode, char Ftype, bool isExtended) { Open(Fname, Omode, Ftype, isExtended); };
...
bool LogFile::Start(const char *message) {
    ...
    snprintf(buf, 256, "%s: %31.31s %s", msg, "Session sucessfully started at", buf2);
    Msg(buf);
    return true;
}
...
bool LogFile::End(bool is_normal) {
    ...
    FJCGetTD(FJC_NEED_TIME | FJC_NEED_DATE, buf2);
    if (is_normal)
        snprintf(buf, 256, "%s: %31.31s %s", msg, "Session sucessfully ended at", buf2);
    else  // Не нормально :=)
