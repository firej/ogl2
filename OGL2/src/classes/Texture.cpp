...
inline int ilLoadImage(const char *filename) {
  ...
  g_image_data = stbi_load(filename, &g_image_width, &g_image_height,
                           &g_image_channels, 0);
  if (g_image_data) {
    printf("Successfully loaded image: %dx%d, %d channels\n", g_image_width,
           g_image_height, g_image_channels);
    return IL_TRUE;
  } else {
...
inline int ilLoadL(ILenum type, void *lump, ILuint size) {
  ...
  g_image_data =
      stbi_load_from_memory((unsigned char *)lump, size, &g_image_width,
                            &g_image_height, &g_image_channels, 0);
  if (g_image_data) {
    return IL_TRUE;
  } else {
    return IL_FALSE;
...
inline int ilTexImage(int width, int height, int depth, int bpp, int format,
                      int type, void *data) {
...
bool TextureClass::Load() {
  ...
  case FJC_TEX_NO_FILTERING: {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, Bpp, iWidth, iHeight, 0, type,
  ...
  case FJC_TEX_TRILINEAR_FILTERING: {
    ...
    //--Делаем мипмапы
    gluBuild2DMipmaps(GL_TEXTURE_2D, Bpp, iWidth, iHeight, type,
                      GL_UNSIGNED_BYTE, data);
...
void FJCRendererTexture::Save() {
  ...
  if (ret == 0)
    MessageBox(NULL, "Error on loading texture into IL", "ERROR!", 16);
  ret = ilGetError();
  if (ret != 0)
    MessageBox(NULL, iluErrorString(ret), "ERROR! in ilTexImage", 16);
  do {
    ScreenIndex++;
  ...
  if (ret != 0)
    MessageBox(NULL, iluErrorString(ret), "ERROR! in ilSaveImage", 16);
}
