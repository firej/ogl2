...
void CConsole::Draw() {
    switch (State) {
        case Console::ST_INVISIBLE:
            return;  // Как можно рисовать консоль в закрытом состоянии?
        case Console::ST_FADEIN:
            persent += pers_step * GT.GetFrameTime() * 60;
            if (persent >= transparency) State = Console::ST_VISIBLE;
            break;
        case Console::ST_FADEOUT:
            persent -= pers_step * GT.GetFrameTime() * 60;
            if (persent <= 0) State = Console::ST_INVISIBLE;
            break;
        default:
            break;
    }
...
void CConsole::ProcessKey(DWORD key) {
    ...
    if (key == VK_UP) {
        if (!CommandsHistory.empty()) {
            if (iCndHistory == CommandsHistory.end())
                ...
                iCndHistory++;
            if (iCndHistory == CommandsHistory.end()) iCndHistory = CommandsHistory.begin();
    ...
    if (key == VK_DOWN) {
        if (!CommandsHistory.empty()) {
            if (iCndHistory == CommandsHistory.begin()) iCndHistory = CommandsHistory.end();
            iCndHistory--;
            list->data.s = (*iCndHistory);
            cursor = (DWORD)list->data.s.size();
        }
...
void CConsole::Show() {
    if ((State != Console::ST_FADEIN) && (State != Console::ST_VISIBLE)) State = Console::ST_FADEIN;
}
void CConsole::Hide() {
    if ((State != Console::ST_FADEOUT) && (State != Console::ST_INVISIBLE)) State = Console::ST_FADEOUT;
}
...
void CConsole::DropHistory() { CommandsHistory.clear(); }
