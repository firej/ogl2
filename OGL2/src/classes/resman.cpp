...
void ResMan::INIT(const char *DefFont, const char *DefTexture,
                  const char *DefMesh) {
...
void ResMan::DEINIT(void) {
  ...
  mapL.clear(); //	Очистка массива низкого уровня, хотя он и должен быть
                // пустым
}

void ResMan::LOAD_Font(const char *ResName, const char *FontName) {
  PFont F;
  Text::RESULT res;
  mapLi =
      mapL.find(string(FontName)); // Поиск елемента в словаре (Есть ли он там?)
  if (mapLi != mapL.end()) {
    mapLi->second.used++;
  ...
  } else { //	Загрузка не удалась, потому пишем указатель на стандартный
           // объект
...
PFont ResMan::SELECT_Font(const char *ResName) {

  if (IsBadReadPtr(ResName, 1)) {
    return defFont;
  }
...
void ResMan::ULOAD_Font(const char *ResName) {
  mapHi = mapH.find(string(ResName));
  if (mapHi == mapH.end()) {
    return;
  }
...
void ResMan::LOAD_Texture(const char *ResName, const char *FileName) {
  TexturePointer T;
  bool res;
  mapLi =
      mapL.find(string(FileName)); // Поиск елемента в словаре (Есть ли он там?)
  if (mapLi != mapL.end()) {
    mapLi->second.used++;
    mapHi = mapH.find(string(ResName));
    if (mapHi != mapH.end()) {
...
TexturePointer ResMan::SELECT_Texture(const char *ResName) {
  if (IsBadReadPtr(ResName, 1))
    return defTexture;
  mapHi = mapH.find(string(ResName));
  if (mapHi == mapH.end())
...
void ResMan::ULOAD_Texture(const char *ResName) {
  mapHi = mapH.find(string(ResName));
  if (mapHi == mapH.end()) {
    return;
  }
...
//===[ MESHES
//]==============================================================================================
void ResMan::LOAD_Mesh(const char *ResName, const char *FileName) {
  Mesh::RESULT res;
  meshPointer M = 0;
  mapLi =
      mapL.find(string(FileName)); // Поиск елемента в словаре (Есть ли он там?)
  ...
  if (res == Mesh::OK) {
    mapH[string(ResName)].Res = M;
           ...
           // объект
    LF.Logf("ResourceManager",
            "Файл '%s' не найден!!! Используем стандартный ресурс", FileName);
    mapH[string(ResName)].Res = defMesh;
  }
}

meshPointer ResMan::SELECT_Mesh(const char *ResName) {
  if (IsBadReadPtr(ResName, 1))
    return defMesh;
  mapHi = mapH.find(string(ResName));
  if (mapHi == mapH.end())
...
void ResMan::ULOAD_Mesh(const char *ResName) {
  mapHi = mapH.find(string(ResName));
  if (mapHi == mapH.end()) {
    return;
  }
...
FindRes_ret_t ResMan::FindRes(ResMapH_Iter_t mapHi) {
  for (ResMapL_Iter_t i = mapL.begin(); i != mapL.end(); i++) {
    if ((i->second.Res == this->defFont) || (i->second.Res == this->defMesh) ||
  ...
  __asm INT 3;
#else
  // macOS/Linux equivalent - trigger debugger breakpoint
  __builtin_trap();
#endif
}
