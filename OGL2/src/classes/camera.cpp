...
void Camera::Look(void) {
    ...
    Sound.SetLOrientation(Vector3f((float)Direction.d.c.x, (float)Direction.d.c.y, (float)Direction.d.c.z),
...
void Camera::MouseLook(void) {
    ...
    if (Angles.d.c.x > 90.0f)
        Angles.d.c.x = 90.0f;
    else if (Angles.d.c.x < -90)
        Angles.d.c.x = -90.0f;
#else
...
void Camera::KeybMove(void) {
    static double speed;
    if (Input::I->get(KeySet.MoreFastMove))
        speed = 3 * CamS.BackwardSpeed;
    else
        speed = CamS.BackwardSpeed;
    if (Input::I->get(KeySet.CamMoveForward)) {
        if (Input::I->get(KeySet.MoreFastMove))
            speed = 3 * CamS.ForwardSpeed;
        else
            speed = CamS.ForwardSpeed;
    ...
    if (Input::I->get(KeySet.CamMoveLeft)) {
        if (Input::I->get(KeySet.MoreFastMove))
            speed = 3 * CamS.StrafeSpeed;
        else
            speed = CamS.StrafeSpeed;
