...
Mesh::RESULT Mesh::simpleMesh::Load(char *FileName) {
    ...
    for (unsigned int loop = 0; loop < mat_q; loop++) {
        ...
        sscanf(szString, "\t\t*MATERIAL_AMBIENT %f %f %f", &material[loop].ambient.d.c.x, &material[loop].ambient.d.c.y,
               &material[loop].ambient.d.c.z);

        FindStr("\t\t*MATERIAL_DIFFUSE", fModel);
        sscanf(szString, "\t\t*MATERIAL_DIFFUSE %f %f %f", &material[loop].diffuse.d.c.x, &material[loop].diffuse.d.c.y,
               &material[loop].diffuse.d.c.z);

        FindStr("\t\t*MATERIAL_SPECULAR", fModel);
        sscanf(szString, "\t\t*MATERIAL_SPECULAR %f %f %f", &material[loop].specular.d.c.x,
               &material[loop].specular.d.c.y, &material[loop].specular.d.c.z);
    ...
    do {
        ...
        if (strstr(szString, "WIREFRAME_COLOR")) {
            ...
            glDisable(GL_TEXTURE_2D);
        } else if (strstr(szString, "*MATERIAL_REF")) {
        ...
        for (i = 0; i < Faces; i++) {
            ...
            glTexCoord2f(u[tfa[i]], v[tfa[i]]);
            glVertex3f(vx[fa[i]], vy[fa[i]], vz[fa[i]]);
            glNormal3f(nx[i], ny[i], nz[i]);
            glTexCoord2f(u[tfb[i]], v[tfb[i]]);
            glVertex3f(vx[fb[i]], vy[fb[i]], vz[fb[i]]);
            glNormal3f(nx[i], ny[i], nz[i]);
            glTexCoord2f(u[tfc[i]], v[tfc[i]]);
            glVertex3f(vx[fc[i]], vy[fc[i]], vz[fc[i]]);
        }
        glEnd();
