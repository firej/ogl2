...
ALvoid SoundController::SetDistModel(ALenum value) { alDistanceModel(value); };
ALvoid SoundController::SetMasterGain(ALfloat MasterGain) { alListenerf(AL_GAIN, MasterGain); };
ALvoid SoundController::SetLOrientation(Vector3f Direction, Vector3f UpVector) {
    LDirection = Direction;
    LUpVector = UpVector;
    ALfloat tempVect[6] = {Direction.d.c.x, Direction.d.c.y, Direction.d.c.z,
                           UpVector.d.c.x,  UpVector.d.c.y,  UpVector.d.c.z};
...
inline ALvoid soundSource::SetPitch(ALfloat pitch) { alSourcef(id, AL_PITCH, pitch); };
inline ALvoid soundSource::SetGain(ALfloat gain) { alSourcef(id, AL_GAIN, gain); };
//====[ Управление воспроизведением ]======================================================================
ALvoid soundSource::Play() {
    now_playing = AL_TRUE;
    alSourcePlay(id);  // this->BufID);
};
ALvoid soundSource::Pause() { alSourcePause(id); };
ALvoid soundSource::Stop() {
    now_playing = AL_FALSE;
    alSourceStop(id);
};
ALvoid soundSource::Rewind() { alSourceRewind(id); };
ALvoid soundSource::SetLooping(ALboolean Looping) { alSourcei(id, AL_LOOPING, Looping); };
