...
#define CAM_LOOKTO_MODE_POINT                                                  \
  1 // --//-- точку на которую толжная указывать камера
#define CAM_LOOKTO_MODE_ANGLES                                                 \
  2 // Задание углов поворота камеры (наибыстрейший метод)
...
namespace EStates {
enum type { RENDER_3D, RENDER_2D, PLAY_CLIP };
}
namespace ERS {
namespace Draw {
enum type { SCENE = 0, TEXTURE = 1, CONSOLE = 2 };
}
namespace Mesh {
enum type { NORMAL = 0, WIRE = 1, HALFWIRE = 2 };
}
} // namespace ERS
...
struct GlobalsStruct // Глобальные настройки и переменные
{
  ...
  unsigned char TextureFiltering; // Тип фильтрации текстур
                                  //	unsigned int	lists;
  //// Количество задействованых списков (попутно и номер последнего)
  struct tagRESstruct {
...
extern KeySettings
    KeySet; // Структура содержащая информацию о функциональных клавишах
...
struct CPUInfo {
  CPUInfo() { memset(this, 0, sizeof(CPUInfo)); }
