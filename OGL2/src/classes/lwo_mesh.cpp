...
Mesh::LWOMesh::LWOMesh() {
  ...
  this->iSurfaces = 0;
  compiled = false;
  list = 0; // Инициализируем OpenGL display list ID
}
...
Mesh::RESULT Mesh::LWOMesh::Load(char *FileName) {
  ...
  filesize = iff::GetDWORD(
      fModel); // Здесь находим размер всего файла с модельками + 8
  ...
  do {
    ...
    switch (tagb.i) {
    case ID_PNTS:
      FASSERT(ReadPNTSChunk(fModel) == FJC_NO_ERROR);
      break;
    case ID_POLS:
      FASSERT(ReadPOLSChunk(fModel) == FJC_NO_ERROR);
      break;
    case ID_LAYR:
      FASSERT(ReadLAYRChunk(fModel) == FJC_NO_ERROR);
      break;
    case ID_TAGS:
      FASSERT(ReadTAGSChunk(fModel) == FJC_NO_ERROR);
      break;
    case ID_SURF:
      FASSERT(ReadSURFChunk(fModel) == FJC_NO_ERROR);
      break;
    case ID_PTAG:
      FASSERT(ReadPTAGChunk(fModel) == FJC_NO_ERROR);
      break;
    default:
      iff::read_unknow_chunk(fModel);
    }
  } while (!feof(fModel));
  PostLoadProcessing();
  fclose(fModel);
...
bool Mesh::LWOMesh::ReadPNTSChunk(FILE *f) {
  ...
  if (((sizeb / 3) * 3) != (sizeb))
    return FJC_ERROR; // Маненькая проверка
  sizeb /= 12;        // Получаем количество точек
  Layers[iLayerCur].iVertexes = sizeb;
  Layers[iLayerCur].Vertexes =
      new lwVertex_t[sizeb]; // Выделяем память под массив вершин
...
bool Mesh::LWOMesh::ReadPOLSChunk(FILE *f) {
  ...
  sizeb >>= 1; // Делим на 2 и получаем количество 2-хбайтных элементов
  posPOLS = ftell(f);
  for (UINT i = 0; i < sizeb;) {
    ...
    lvertexes <<= 1;    // Умножаем на 2
    fseek(f, lvertexes,
          SEEK_CUR); // Сдвигаем указатель в файле на количество вершин
    pols++;          // Считаем полигоны
  }
  fseek(f, posPOLS, SEEK_SET); // Откат указателя назад (в начало POLS)

  Layers[iLayerCur].iPols = pols;
  Layers[iLayerCur].Pols =
      new Mesh::Poly_t[pols]; // Выделяем память под полигоны
  ...
  for (UINT i = 0; i < pols; i++) {
    ...
    Layers[iLayerCur].Pols[i].ns =
        new bool[lvertexes]; // Выделяем память под флаги
...
bool Mesh::LWOMesh::ReadSURFChunk(FILE *f) {
  ...
  for (; ReadedSize < SurfChunkSize; ReadedSize += ChunkSize) {
    ...
    switch (tagb.i) {
    case ID_COLR:
      ChunkSize = ReadSURFCOLRSubChunk(f);
      break;
    case ID_DIFF:
      ChunkSize = ReadSURFDIFFSubChunk(f);
      break;
    case ID_SPEC:
      ChunkSize = ReadSURFSPECSubChunk(f);
      break;
    case ID_TRAN:
      sizeb = iff::GetWORD(f);
      float alfa;
      ...
      if (alfa > 1)
        alfa = 1.0f; // Маленькие ограничения на прозрачность
      if (alfa < 0)
        alfa = 0.0f;
    ...
    default:
      ChunkSize = iff::read_unknow_subchunk(f);
    }
  }
  return FJC_NO_ERROR;
}
...
void Mesh::LWOMesh::PostLoadProcessing(void) {
  ...
  // Смотрим соответствие имени в теге и имени сурфейса и соответственно пишем
  // тегу номер сурфейса
  ...
  //  Первым делом для каждой вершины слоя создаём список полигонов, которым она
  //  принадлежит
  for (DWORD ind = 0; ind < iLayers; ind++) {
    lwLayer_t *l = &Layers[ind];
    ...
    for (DWORD j = 0; j < l->iVertexes; j++)
      vert[j].n = j;
    // Записываем для каждой вершины её полигоны
    for (DWORD i = 0; i < l->iPols; i++) {
      if (l->Pols[i].v < 3)
        continue; // полигоны меньше 3х точек нам не нужны
    ...
    for (DWORD i = 0; i < l->iVertexes; i++) // Цикл по всем вершинам
    {
      ...
      for (DWORD k = 0; k < vert[i].polys.size(); k++) {
        for (DWORD li = k; li < vert[i].polys.size(); li++) {
          if (li == k)
            continue;
          // Если полигоны принадлежат разным материалам или материал без
          // сглаживания мы их не обрабатываем
          if (vert[i].polys[k].p->iSurf != vert[i].polys[li].p->iSurf ||
              SurfList[vert[i].polys[li].p->iSurf].Smoothing == false)
            continue;
          // Теперь находим угол между нормалями полигонов (этот угол
          // используется для решения - сглажывать или нет)
          float surfAngle = SurfList[vert[i].polys[li].p->iSurf].nSmoothAngle;
          float polyAngle = (float)vert[i].polys[k].p->normal.angle(
              vert[i].polys[li].p->normal);
          if (polyAngle > 0 &&
              polyAngle < surfAngle) { // Угол между нормалями меньше
                                       // максимального для текущего материала
            // Значит этот полигон ещё не внёс свой вклад в результирующую
            // нормаль
    ...
    for (DWORD i = 0; i < l->iPols; i++) {
      if (CurrentSurface != l->Pols[i].iSurf) {
        ...
        glMaterialfv(GL_FRONT, GL_DIFFUSE,
                     SurfList[CurrentSurface].Diffuse.d.v);
        glMaterialfv(GL_FRONT, GL_SPECULAR,
                     SurfList[CurrentSurface].Specular.d.v);
      }
      glBegin(GL_POLYGON);
