...
public:
	...
	float x;
	float y;
	TextBlock(	Font *fontp,	char* textp,	float xp,	float yp)
	{
		font	=	fontp;
		text	=	textp;
		x		=	xp;
...
public:
	StartUPLogo()
	{};
	bool init(	void *SwapBuffersFunc,
				unsigned int logo_file_type = IL_PNG,
	...
	void render_logo(	unsigned char mode,
						int text_blocks = 0,
					...
					)
	{
		...
		static int	PointsArray[12]=	{vp[0],vp[1],0,
										 vp[2],vp[1],0,
										 vp[2],vp[3],0,
										 vp[0],vp[3],0 };
		static int	TexCoordArray[8]=	{0,1,
										 1,1,
										 1,0,
										 0,0};
		...
		case FJC_STARTUP_LOGO_MODE_PROCEED_WITH_TEXT:
			glClear(GL_COLOR_BUFFER_BIT);				// Очистка буфера цвета
				...
				for ( int i = 0,p_shift = 0; i<text_blocks ; i++,p_shift = i*sizeof(TextBlock))
					(&blocks+p_shift)->font->Print(	(&blocks+p_shift)->x,
													(&blocks+p_shift)->y,
													(&blocks+p_shift)->text
												  );
			SBFunc();
