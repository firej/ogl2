...
public:
  TextureLogo(){};
  bool init(unsigned int logo_file_type = IL_PNG,
  ...
  void render_logo(float scale = 0.5, float alpha = 0.5, bool left = false,
                   bool top = true) {
    GLint vp[4];
    glPushAttrib(GL_DEPTH_BUFFER_BIT | GL_TEXTURE_BIT | GL_ENABLE_BIT |
                 GL_LIGHTING_BIT);
...
#endif
#define FJC_STARTUP_LOGO_TIME_TO_START                                         \
  300 // Время появления/затухания в миллисекундах
...
public:
  ...
  void render_logo(unsigned char mode, int text_blocks = 0,
                   TextBlock blocks = TextBlock(NULL, NULL, 0, 0), ...) {
    GLint vp[4];
    ...
    static int PointsArray[12] = {vp[0], vp[1], 0, vp[2], vp[1], 0,
                                  vp[2], vp[3], 0, vp[0], vp[3], 0};
    ...
    case FJC_STARTUP_LOGO_MODE_BEGIN:
      for (int i = 0; i < FJC_STARTUP_LOGO_STEPS_TO_BLEND; i++) {
        ...
        glColor4f(1.0f, 1.0f, 1.0f,
                  float(i) / (float)FJC_STARTUP_LOGO_STEPS_TO_BLEND);
    ...
    case FJC_STARTUP_LOGO_MODE_PROCEED_WITH_TEXT:
      ...
      glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
      glDrawArrays(GL_QUADS, 0, 4);
      for (int i = 0, p_shift = 0; i < text_blocks;
           i++, p_shift = i * sizeof(TextBlock))
        (&blocks + p_shift)
            ->font->Print((&blocks + p_shift)->x, (&blocks + p_shift)->y,
                          (&blocks + p_shift)->text);
      SBFunc();
      break;
    }
