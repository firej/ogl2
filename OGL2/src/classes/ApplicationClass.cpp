...
bool PeekMessage(MSG *msg, void *hwnd, unsigned int min, unsigned int max,
                 unsigned int remove) {
  return false;
}
bool GetMessage(MSG *msg, void *hwnd, unsigned int min, unsigned int max) {
  return false;
}
...
Colormap XCreateColormap(Display *display, Window w, void *visual, int alloc) {
  return 0;
}
...
Window XCreateWindow(Display *display, Window parent, int x, int y,
                     ...
                     unsigned int class_type, void *visual,
                     unsigned long valuemask,
                     XSetWindowAttributes *attributes) {
  return 0;
}
void XMapWindow(Display *display, Window w) {}
bool XIfEvent(Display *display, XEvent *event_return, bool (*predicate)(),
              char *arg) {
  return false;
}
...
// ]===================================================================================
  ...
  glGetIntegerv(GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT,
                &SysInfo.video.MaxAnisotropy);
  SysInfo.video.RendererName =
      (char *)glGetString(GL_RENDERER); // Выводим имя карты
  LF.Logf("GetSysInfo", "Render            %s", SysInfo.video.RendererName);
  SysInfo.video.VendorName =
      (char *)glGetString(GL_VENDOR); // Выводим имя поставщика
  LF.Logf("GetSysInfo", "Vendor            %s", SysInfo.video.VendorName);
  SysInfo.video.OpenGL_Version =
      (char *)glGetString(GL_VERSION); // Выводим версию
  ...
  LF.Logf("GetSysInfo", "OpenGL version is %s", SysInfo.video.OpenGL_Version);
  SysInfo.video.SupportedGlExtentions =
      (char *)"GL_EXT_texture_filter_anisotropic";
#endif
// ===[ Процессор::реестр
// ]=======================================================================================
#ifdef WIN32
  HKEY CPinfo;
  FASSERT(!RegOpenKeyEx(HKEY_LOCAL_MACHINE,
                        "HARDWARE\\DESCRIPTION\\System\\CentralProcessor\\0",
                        NULL, KEY_QUERY_VALUE, &CPinfo));
  DWORD bufsize = sizeof(SysInfo.cpu.speed);
  RegQueryValueEx(CPinfo, "~MHz", NULL, NULL, (LPBYTE)&SysInfo.cpu.speed,
                  &bufsize);
  LF.Logf("GetSysInfo", "CPU speed is %d (by M$ Windows)", SysInfo.cpu.speed);
  bufsize = sizeof(SysInfo.cpu.id);
  RegQueryValueEx(CPinfo, "Identifier", NULL, NULL, (LPBYTE)&SysInfo.cpu.id,
                  &bufsize);
  LF.Logf("GetSysInfo", "CPU identifier \"%s\"", SysInfo.cpu.id);
  bufsize = sizeof(SysInfo.cpu.name);
  RegQueryValueEx(CPinfo, "ProcessorNameString", NULL, NULL,
                  (LPBYTE)&SysInfo.cpu.name, &bufsize);
  LF.Logf("GetSysInfo", "CPU name  \"%s\" ", SysInfo.cpu.name);
  bufsize = sizeof(SysInfo.cpu.vendor_id);
  RegQueryValueEx(CPinfo, "VendorIdentifier", NULL, NULL,
                  (LPBYTE)&SysInfo.cpu.vendor_id, &bufsize);
  ...
  SysInfo.cpu.MMX =
      (char)IsProcessorFeaturePresent(PF_MMX_INSTRUCTIONS_AVAILABLE);
  if (SysInfo.cpu.MMX)
    strcat(fichi, " MMX");
  SysInfo.cpu.AMD_3DNow =
      (char)IsProcessorFeaturePresent(PF_3DNOW_INSTRUCTIONS_AVAILABLE);
  if (SysInfo.cpu.AMD_3DNow)
    strcat(fichi, " AMD_3DNow");
  SysInfo.cpu.RDTSC =
      (char)IsProcessorFeaturePresent(PF_RDTSC_INSTRUCTION_AVAILABLE);
  if (SysInfo.cpu.RDTSC)
    strcat(fichi, " RDTSC");
  SysInfo.cpu.SSE =
      (char)IsProcessorFeaturePresent(PF_XMMI_INSTRUCTIONS_AVAILABLE);
  if (SysInfo.cpu.SSE)
    strcat(fichi, " SSE");
  SysInfo.cpu.SSE2 =
      (char)IsProcessorFeaturePresent(PF_XMMI64_INSTRUCTIONS_AVAILABLE);
  if (SysInfo.cpu.SSE2)
    strcat(fichi, " SSE2");
  ...
  SysInfo.cpu.speed =
      int((float)ticks / (TIME_TO_CALCULATE_CPU_SPEED * 1000.0f));
...
#pragma warning(disable : 4127)
  if (CPU_features & RDTSC_ENABLED)
    SysInfo.cpu.RDTSC = true;
  if (CPU_features & MMX_ENABLED)
    SysInfo.cpu.MMX = true;
  if (CPU_features & SSE_ENABLED)
    SysInfo.cpu.SSE = true;
  if (CPU_features & SSE2_ENABLED)
    SysInfo.cpu.SSE2 = true;
  if (CPU_features & AMD_3D_NOW_ENABLED)
    SysInfo.cpu.AMD_3DNow = true;
  if (CPU_features & AMD_3D_NOW_Ex_ENABLED)
    SysInfo.cpu.AMD_3DNow_Ex = true;
#pragma warning(default : 4127)
  ...
  if (SysInfo.cpu.AMD_3DNow)
    strcat(fichi, " AMD_3DNow");
  if (SysInfo.cpu.AMD_3DNow_Ex)
    strcat(fichi, " AMD_3DNow_Ex");
  if (SysInfo.cpu.RDTSC)
    strcat(fichi, " RDTSC");
  LF.Logf("GetSysInfo", "%s", fichi);
#endif
...
bool Application::ReadConfig() {
  ...
  Globals.VP.ScreenStrings =
      40; // Установка количества строк на экране (для консоли)
  ...
  do {
    ...
    do {
      ...
      if (strcmp(name, "fullscreen") ==
          0) { // запускать в полноэкранном режиме?
...
bool Application::LoadResources() {
  ...
  if (loadedMesh) {
    ...
    LF.Logf("LoadResources", "ERROR: Failed to load mesh '%s' from file '%s'",
            "someMesh", ModelName);
  }
...
bool Application::Birth() {
  ...
  wc.hIcon =
      LoadIcon(Application::Vars.App.HInstance, MAKEINTRESOURCE(IDI_ICON1));
  wc.hIconSm =
      LoadIcon(Application::Vars.App.HInstance, MAKEINTRESOURCE(IDI_ICON1));
  ...
  if (IsFullScreen) {
    EnumDisplaySettings(NULL, ENUM_CURRENT_SETTINGS,
                        &Application::Vars.win.PrevDS);
    memset(&Application::Vars.win.CurrDS, 0,
           sizeof(Application::Vars.win.CurrDS));
    ...
    Application::Vars.win.CurrDS.dmFields =
        DM_BITSPERPEL | DM_PELSWIDTH | DM_PELSHEIGHT | DM_DISPLAYFREQUENCY;
    Application::Vars.win.CurrDS.dmDisplayFrequency = Globals.VP.Hzs;

    if (ChangeDisplaySettings(&Application::Vars.win.CurrDS, CDS_FULLSCREEN) !=
  ...
  Application::Vars.win.MHWnd = CreateWindowEx(
      dwExStyle, MY_WINDOW_CLASS_NAME, MY_WINDOW_NAME,
      dwStyle | WS_CLIPSIBLINGS | WS_CLIPCHILDREN, WindowRect.left,
  ...
  // Создание окна
  glfwWindow = glfwCreateWindow(Globals.VP.Width, Globals.VP.Height,
                                "OGL2 Engine", NULL, NULL);
  if (!glfwWindow) {
  ...
  SimpleLogo.init((void *)SwapBuffersEXT, IL_PNG,
                  "data/textures/Logos/NewLOGO.PNG");
  ...
  SimpleLogo.render_logo(FJC_STARTUP_LOGO_MODE_PROCEED_WITH_TEXT, 1,
                         TextBlock(rm.SELECT_Font("Consolas"),
                                   "Now loading, please wait", 250.0f, 300.0f));
  Sound.Init();
  TestBuf.Init();
...
bool Application::Suicide() {
  ...
  if (Application::Vars.win.RC) {
    ...
    wglDeleteContext(
        Application::Vars.win.RC); // Delete our OpenGL Rendering Context
  }
...
bool Application::ReshapeWindow(int width, int height) {
  if (height == 0)
    height = 1; // Чтобы избежать деления на ноль
...
void Application::ProcessKBInput() {
  ...
  if (CCons.Visible())
    return; // Если консоль видима, оставляем все кнопочки ей
...
void Application::ShowMovie(bool CanBreak) {
  ...
  do { // Обработка всех сообщений
    ...
    SwapBuffersEXT();
    if (CanBreak) {
      if (Input::I->get(' ') || Input::I->get(VK_RETURN) ||
